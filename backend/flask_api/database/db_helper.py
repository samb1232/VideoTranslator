...
class DbHelper:
    ...
    def add_users(self, users: List[dict]):
        # TODO: Изменить код так, чтобы старые юзеры удалялись + добавить проверку на одинаковые пароли.
        try:
            session = self._get_session()
            
            self.check_users_format(users)
    ...
    @staticmethod
    def check_users_format(users: List[dict]) -> bool:
        for user in users:
            if "username" not in user or "password" not in user:
                raise KeyError("Invalid users format")
    ...
    def _get_task_by_id_with_session(self, task_id: str, session: Session) -> Task:
    ...
    def delete_task_by_id(self, task_id: str) -> bool:
        ...
        session = self._get_session()
        try:
            task = self._get_task_by_id_with_session(task_id, session)
            if task is None:
                return False
            session.delete(task)
            session.commit()
