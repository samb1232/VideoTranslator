...
def create_blueprint(
    ...
    def get_all_tasks():
        tasks_arr = db_helper.get_all_tasks_list()
        tasks = []
    ...
    def get_task(id):
        task = db_helper.get_task_by_id(id)
        if task is None:
    ...
    def create_task():
        ...
        task = db_helper.create_new_task(title=title, creator_username=creator_username)
        return jsonify({'status': 'success', "task_id":  task.id}), 200


    @bp.route('/delete_task/<id>', methods=['DELETE'])
    def delete_task(id):
        is_success = db_helper.delete_task_by_id(id)
        if is_success:
            return jsonify({'status': 'success'}), 200
        return jsonify({'status': 'error', "message": "Task not found"}),  404
    ...
    def download_file(filepath):
        ...
        if not os.path.exists(filepath):
            return jsonify({'status': 'error', "message":  f"File {filepath} not found"}), 404
        
        return send_from_directory("./", filepath, as_attachment=True), 200
    ...
    def create_subs():
        try:
            ...
            db_helper.set_task_src_vid_path(task_id, vid_filepath)
            db_helper.set_task_languages(task_id, lang_from, lang_to)
